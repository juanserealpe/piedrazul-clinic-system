...
import { getAppointments, exportAppointmentsCsv, reScheduleAppointment } from "@/services/appointment.service";
...
export default function SchedulerAppointmentsPage() {
  ...
  const [reagendarData, setReagendarData] = useState<{ id: string; fecha: string } | null>(null);
  ...
  const handleConfirmReagendar = async (appointmentId: string, newDate: string) => {
    try {
      await reScheduleAppointment({ appointmentId, newDate: `${newDate}T00:00:00.000Z` });
      alert("Cita reagendada correctamente");
      setReagendarData(null);
      loadCitas();
    } catch (e) { console.error(e); alert("Error al reagendar"); }
  };
  ...
  return (
    <div>
                        ...
                        <td style={{ textAlign: "center" }}>
                          <button
                            onClick={() => setReagendarData({
                              ...
                              fecha: new Date(apt.date).toLocaleDateString("es-CO", { timeZone: "UTC", day: "2-digit", month: "long", year: "numeric" }),
                            })}
                            style={{
                              background: "var(--pz-amber-light)", color: "var(--pz-amber)",
                              border: "2px solid #f6d87a", borderRadius: "8px",
                              padding: "7px 14px", fontWeight: 700, fontSize: "0.85rem", cursor: "pointer",
                            }}
                          >
                            Reagendar
                          </button>
      ...
      {tab === "crear" && (
        <div>
          <PatientTable onCreateAppointment={(patient) => {
